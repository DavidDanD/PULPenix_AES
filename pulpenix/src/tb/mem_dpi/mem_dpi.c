...
void mem_socket_open() {
  ...
  if(bind(mem_socket_m, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
    fprintf(stderr, "Unable to bind the socket: %s\n", strerror(errno)); // TMP FIX UDI
           //NOTICE COMP WARN: $TESTDIR/../rtl/tb/mem_dpi/mem_dpi.c:204:5: warning: format '%s' expects argument of type 'char *', but argument 3 has type 'int' [-Wformat=]
    return;
  }
  ...
  fprintf(stderr, "mem_dpi: Listening on port %d\n", mem_socket_port);
}

void mem_reopen() {
  close(mem_socket_c);
  // mem_socket_open(mem_socket_port);
  mem_socket_open(); // TMP MODIFYIED BY UDI mem_socket_open() takes no param , uses global mem_socket_port
}
...
int mem_poll(mem_packet_t* packet, char* buffer) {
  ...
  int ret;
  int received;
  //printf("TMP_DBG from mem_dpi.c: Entered mem_poll\n") ;
  if(!mem_got_con) {
    if(!mem_check_con()) {
      return -1;
    }
...
void mem_init(const int port) {
  ...
  printf("Entering mem_dpi.c mem_init port=%d\n",port);

...
// Added to fix single step issue per https://lists.ee.ethz.ch/wws/arc/pulp-info/2017-05/msg00011.html

void  Rsp::resumeCoreStep(DbgIF* dbgif, bool step) {
   uint32_t hit;
   uint32_t cause;
   uint32_t ppc;
   uint32_t npc;
...
// End of - "Added to fix single step issue"
...
Rsp::v_packet(char* data, size_t len) {
  ...
  // TMP ADDED BY UDI - SEEMS AS EXPEXTED FROM GDB CLIENT 
  else if (strncmp ("vMustReplyEmpty", data, strlen ("vMustReplyEmpty")) == 0)
  {
    //printf("TMP_DBG GOT $vMustReplyEmpty\n");
    return this->send_str("");
  }
...
// START OF PATCH CODE BY UDI
// Alternative to: https://www.bountysource.com/issues/42139976-register-834-is-not-available-error-for-breakpoint
  else if (addr==834) {
   //printf ("TMP_DBG: Access reg 834 patch\n"); 
   rdata = 0 ;   
  }     
// END OF PATCH CODE BY UDI
  else
    return this->send_str("");
...
Rsp::get_packet(char* pkt, size_t* p_pkt_len) {
  ...
  do {
    ...
    if((ret == -1 && errno != EWOULDBLOCK) || (ret == 0)) {
      fprintf(stderr, "RSP: Error receiving (#1) ret=%d , errno=%d  \n",ret,errno);
      return false;
    }
  ...
  do {
    ...
    if((ret == -1 && errno != EWOULDBLOCK) || (ret == 0)) {
      fprintf(stderr, "RSP: Error receiving (#2)\n");
      return false;
    }
  ...
  if((ret == -1 && errno != EWOULDBLOCK) || (ret == 0)) {
    fprintf(stderr, "RSP: Error receiving (#3)\n");
    return false;
  }

  ret = recv(m_socket_client, &check_chars[1], 1, 0);
  if((ret == -1 && errno != EWOULDBLOCK) || (ret == 0)) {
    fprintf(stderr, "RSP: Error receiving (#4)\n");
    return false;
  }
...
Rsp::send(const char* data, size_t len) {
  ...
  do {
    ...
    if((ret == -1 && errno != EWOULDBLOCK) || (ret == 0)) {
      free(raw);
      fprintf(stderr, "RSP: Error receiving (#5)\n");
      return false;
    }
...
/*  Replaced with belowe to fix single step issue per https://lists.ee.ethz.ch/wws/arc/pulp-info/2017-05/msg00011.html
bool
...
bool Rsp::resume(bool step) {
   if (m_dbgifs.size() == 1) {
      int ret;
      char pkt;
      DbgIF *dbgif = this->get_dbgif(m_thread_sel);
      
      resumeCoreStep(dbgif, step);
...
Rsp::mem_read(char* data, size_t len) {
  ...
  for(i = 0; i < length; i++) {
    // rdata = buffer[i]; 
    rdata = (unsigned char) buffer[i]; // MODIFIED BY UDI - CAST ADDED otherwise gets FF fr negative chars

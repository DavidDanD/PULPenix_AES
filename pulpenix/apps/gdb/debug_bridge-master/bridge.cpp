...
Platforms platform_detect(MemIF* mem) {
  ...
  //printf("TMP-DBG Entered platform_detect\n");
  
  mem->access(0, 0x10000000, 4, (char*)&info);
  printf("platform_detect info=%x\n",info) ;
...
printf("Initializing Bridge...\n"); //TMP DBG UDI
  ...
  printf("Calling new Rsp(1234 ...\n"); //TMP DBG UDI
  rsp = new Rsp(1234, mem, this->log, dbgifs, bp);
  printf("Done calling new Rsp(1234 ...\n"); //TMP DBG UDI
}
